...
### 新增（真实视频质量修复：配置统一、时序保真、遮挡与字形证据）

围绕真实素材（手机聊天屏、滚动邮件屏、4K OP、手部遮挡邮件屏）逐帧复核
发现并修复的六类缺陷（V1–V7），管线核心新增八个可单测模块（方案与交付
记录见 [docs/FINAL_OPTIMIZATION_DESIGN.md](docs/FINAL_OPTIMIZATION_DESIGN.md)
与 [docs/FINAL_IMPLEMENTATION_PLAN.md](docs/FINAL_IMPLEMENTATION_PLAN.md)）：

- **ROI 运行时选项统一解析（C1）**：新增 `core/roi_runtime_config.py`——
  `--lang` / `--scene-text-policy` 等 CLI 选项缺省改为「跟随 ROI 配置」
  （显式传参仍优先），策略/语言/姿态标签/自动亮度/遮挡开关在**所有处理
  阶段之前**一次性解析，CLI 与 GUI 共用同一解析结果，消除「同一配置在
  不同阶段读到不同值」的整类问题。每 ROI 识别语言与 2.7.3 语义兼容。
- **共存行不再被截为零时长（A1）**：同屏多行字幕（聊天记录等）此前会被
  事件合并当作顺序字幕截短、产生上千条零时长事件——同一 ROI 的事件仅在
  起点、标签完全一致时才做端点齐平，Scene 事件不参与；实测 1441 条
  Dialogue 全保留、零时长事件 1138 → 0。
- **策略按活动集合分段（A2）**：新增 `core/scene_timeline.py`——外置
  展示框等场景策略按文字的**实际活动时间片**布局输出（歧义同文保留原
  事件并留痕），不再把整段时间压成一条；实测某样本活动行 1 → 50 条。
- **轨迹接管需时间保真证据（A3）**：新增 `core/trajectory_fidelity.py`——
  轨迹事件接管静态行前必须通过逐区间内容保真比对（比较窗口归一到静态
  证据跨度、端点对称去噪），证据不足时**拒绝接管**并保留静态输出，消灭
  「把已滚出的标题行冻结到整段」的错误接管形态。
- **遮挡检测消除二次漏采 + 离散逐帧切片（B1/B2）**：新增
  `core/occlusion_timeline.py`——遮挡采样结果经 side-channel 全量保留
  （废除二次七帧抽样这一漏采源），`\iclip` 按命中帧离散切片、锚定当前
  帧单应，不再把蒙版拉伸到无证据区间；步进渐隐的 hold 绝对边界保持不变。
- **有色遮罩与屏幕亮度绑定（B3）**：新增 `core/scene_brightness.py`——
  文字重渲与有色遮罩共用同一背景亮度测量（排除前景、小样本不出数），
  静态遮罩按亮度曲线写 `\1c`（不写 `\alpha`），暗屏下遮罩不再比背景亮。
- **手部前景轮廓逐帧 `\iclip`（B4）**：新增 `core/occluder_contours.py`——
  种子区域生长 + RECOMP 轮廓细化保留指缝洞（顶点数封顶），仅在平面检测
  命中窗口（±5 帧）内细化，可选 1px 外扩；手部遮挡时字幕只被手挡住的
  部分不再渲染文字碎片。实测邮件屏第 8–10 秒 140 处裁剪全部集中在手部
  窗口（指缘少量残留如实记为部分达标）。
- **字形证据判定文字自身运动（D1）**：新增 `core/text_motion_evidence.py`
  ——参考帧字形掩码 + 掩码模板匹配优先判定「文字动没动」，移动背景上
  的静止文字漂移 ≤1px 不再被背景拖出假 `\move`；证据不足回退整块模板
  匹配并留痕（真实小字仍有较多回退，见局限）。
- **按行框约束场景文字字号（C2）**：新增 `core/line_geometry.py`——
  Scene 行无模板时按识别行框拟合逐行 `\fs`（多行事件取各行最小值），
  字号不再超出原文字实际大小；传入样式模板时不干预，样式控制权保留。
- **静止画面遮挡窗口门控**：静态画面证据的遮挡采样限定在平面检测命中
  窗口内、噪声对称归一，聊天屏 ROI 后段不再出现无证据的局部 `\iclip`。

### 修复（管线健壮性）

- **并行 ROI 失败时先停兄弟任务再收尾**：某个 ROI 任务失败后其余任务
  不再继续空跑占用资源。
- **LLM SDK 重试不再乘翻退避预算**：OpenAI SDK 内部重试与 tenacity 有界
  退避叠加导致实际等待远超预算——SDK 侧重试归 1，预算由统一退避层控制
  （429 语义：至多 10 次、单次 ≤60s、总预算 ≤300s，尊重 Retry-After）。
- **ASS 文件原子写出**：新增 `utils/atomic_write.py`——字幕文件完整写
  成后原子替换，中断/崩溃不再留下半截 `.ass`。
- **解码提前终止**：所有 ROI 的最后一个活动帧之后不再继续解码，长视频
  尾段黑屏不再空耗时间。
- **CLI 阶段共享与资源清理**：GUI/CLI 共用同一套阶段装配，OCR 引擎在
  每条退出路径（含异常）上都释放。

### 测试

- 全量回归 **1726 passed, 1 skipped**（较 2.7.4 新增约 158 项：遮挡
  timeline/轮廓/渲染回归、轨迹保真、场景时间线、行几何、ROI 运行时
  配置、原子写出、阶段生命周期等）；`ruff check --select F` 全绿；
  libass 渲染回归覆盖遮挡裁剪与行级字号（字形减面积 ≥80%、字号层级
  保持）；五条真实视频端到端 exit 0（交付记录含逐样本 Dialogue/零时长/
  iclip/move 统计与已知局限）。
...
- **lint 门禁回归修正（发布终验轮）**：`ruff check . --select F` 回归
  全绿——yuzu torch 探测导入补 `# noqa: F401` 显式标注（保留
  `torch_import_failed` 优雅报错路径，清除无效 `# noqa: WPS433`）、
  翻译缩译指令构造删除未使用变量、测试文件未用导入/变量清理；全部
  为零行为变化清理，1561 项单测修正前后零翻转。
...
- **步进/静止体制分段（`core/step_segmentation.py`，默认开）**：轨迹管线
  此前只有「全程静止吸附」与「跟随校正轨迹输出 `\move`」两种归宿，聊天
  界面等「显示一段时间 → 瞬间换位 → 再显示一段时间」的内容两头不靠——
  文字只在一段期间可见（首末采样失配，静止吸附永不触发），实测又证明
  显示期间文字纹丝不动，静止期被输出成漂移的 `\move`（11.mp4 静止期
  中位漂移 24px、12.mp4 一条链 48s 漂近 120px）。现消费 pose-verify 的
  实测采样（`LineVerifyReport` 新增 `measured`：有效采样帧的实测中心，
  本就与静止吸附同源，零额外匹配），把每行可见时间轴划分为 hold 段：
  相邻实测中心位移超换位阈值（`step_jump_px`，0=自动）开新 hold；hold
  段划分**整条 ok 时间轴**（换位后到 B 侧首采样之间不留无字幕空洞），
  拆成 per-hold 恒位姿轨迹交回既有合成器输出逐段 `\pos`——静止是 K=1
  的特例。边界默认取相邻采样帧中点，有视频时**定向加密**精化（粗扫
  `densify_coarse_stride` 帧距括区间 → 命中点 ±`densify_halfwin_frames`
  逐帧精扫，精度 ±1 帧；单次换位解码帧数 `densify_max_frames` 封顶，
  超预算自动放大粗扫帧距；采样间横着 ok 帧断口/相邻采样帧的边界视同
  已确认）。时间上连续的相邻 hold 事件注入 `\fad` 渐隐
  （`step_fade_ms`，默认 100ms、0=纯硬切），消除换位瞬间跳变突兀感，
  与 `\t(\alpha)` 亮度链乘法叠加共存（libass 实渲染验证）。事件
  `line_idx` 合成后重映射回原始行号，遮挡蒙版/场景策略/逐行亮度不受
  拆分影响；对齐投票改按原始行集合进行（`synthesize_events` 新增仅
  关键字参数 `align_boxes`/`line_align_index`，缺省恒等、行为不变）。
  新增 MotionAssConfig 参数（`--config-json` 可调）：
  `hold_pos_segmentation` / `hold_tol_px` / `step_jump_px` /
  `min_hold_samples` / `step_fade_ms` / `drift_recheck_samples` /
  `densify_coarse_stride` / `densify_halfwin_frames` /
  `densify_max_frames`。
- **复核补采（漂移复核 + 短段凑证）**：① 段内偏差超 `hold_tol_px` 的
  候选段先在其采样跨度内均匀补采 `drift_recheck_samples` 个帧重测——
  稀疏采样下模板匹配噪声常呈现为 3~5px「假漂移」，复核后收敛仍是
  hold、仍漂移才整行回退（11.mp4 两条 3~4px 假漂移线其中一条复核后
  确认真漂移回退，另一类边界加密后照常拆分）；② 样本数不足
  `min_hold_samples` 的**内部**短段在采样帧 ±halfwin 邻域补采凑证据
  ——逐场景出现的时间戳/短句每个可见期往往只被采到一次，补采把
  「一瞥」扩成有厚度的 hold（首末段贴着视频首尾淡入淡出瞬态区，不
  探测）。补采帧按段中位中心小半径模板匹配，分数达 `verify_min_score`
  才采纳。
- **连续运动判别（匀速直线签名）**：段内换位向量方向一致、幅度近似
  相等（偏差 ≤ max(35%, 6px×分辨率缩放)且同向）是匀速滚动的采样形态
  ——整行回退既有 `\move` 路径。跨可见断口的跳变是天然边界（逐场景
  出现的文本），不参与判别。DMG 邮件屏（真滚动）回归输出与 2.7.1
  **逐字节一致**；11/12.mp4 的翻页/跨场景跳变方向杂乱，不受影响。

### 实测效果（真实视频回归）

- 11.mp4（LINE 聊天轮播，0-400 帧轨迹产物）：`\move` 70 → 30、`\pos`
  10 → 16；6 行按翻页拆分为逐段 `\pos`（如「帰ってくると思ってなくて」
  3 个 hold，加密定位的边界帧 198/336 与 8.25s/14s 翻页吻合），静止期
  不再漂移。
- 12.mp4（LINE 场景轮播全片，overlap）：`\move` 179 → 158、`\pos`
  19 → 44；10 行拆分（如「彼女さん」3 个逐场景 `\pos`、边界帧 433 由
  加密定位）；时间戳类极短可见期文本按保守口径回退既有路径（不劣化）。
- DMG（真滚动邮件屏）与全量单测：1037 passed（新增 15 项
  `tests/test_step_segmentation.py`），DMG 轨迹产物逐字节一致。

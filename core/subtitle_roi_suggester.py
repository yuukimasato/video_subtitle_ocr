...
def cluster_poly_points_if_tilted(
    ...
    对全部 quad 点求最小外接旋转矩形，用长边向量测量相对最近坐标轴的偏角
    （与 OpenCV 版本的角度约定无关）。偏角 <= ``POLY_TILT_THRESHOLD_DEG`` 返回
    ``None``（调用方维持原轴对齐 rect 路径）；否则把旋转矩形沿长边外扩
    ``X_PAD_PX``、短边外扩 ``y_pad`` 后返回 4 个顶点（逐点钳制到画面内）。
    """
    try:

...
def track_plane_frames(
    ...
    *,
    ...
    pts0, desc0 = _detect_features(gray0, feature_count)
    ...
    near0 = (pts0[:, 0] >= region0[0]) & (pts0[:, 0] <= region0[2]) \
        & (pts0[:, 1] >= region0[1]) & (pts0[:, 1] <= region0[3])
    if int(near0.sum()) < min_matches:
        raise ValueError(
            f"init quad has too few detectable features ({int(near0.sum())} < {min_matches}); "
            "pick a textured plane region"
        )
    ref = _RefState(gray0, pts0, desc0, quad0.copy(), np.eye(3, dtype=np.float64), start_frame_num)

    tracks: List[TrackedQuad] = [
        TrackedQuad(
            frame_num=start_frame_num,
            time_sec=(times[0] if times else float(start_frame_num) / (fps or 30.0)),
            status="ok",

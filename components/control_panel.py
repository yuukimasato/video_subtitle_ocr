...
from PySide6.QtCore import Signal, QCoreApplication, Qt, QThread, QTimer, QSettings
...
class ControlPanelWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.ocr_lang_combo.setToolTip(
            QCoreApplication.translate(
                "ControlPanelWidget",
                "选择字幕语言。中文简体/繁体/英语/日语及拉丁语系使用 PP-OCRv6 模型，"
                "韩/俄/阿拉伯语等自动回落 PP-OCRv5 多语言模型。",
            )
        )
        ...
        # ── AI 翻译（T2.5；可折叠，默认收起 = 关闭）。选项经
        # get_pipeline_options() 交 pipeline_control 组装 TranslationConfig，
        # 关闭时翻译配置为 None（零行为变化）。
        self.translation_group, _translation_content, translation_content = (
            self._make_collapsible_group(
                QCoreApplication.translate("ControlPanelWidget", "AI 翻译（可选）")
            )
        )
        self.translation_group.setToolTip(
            QCoreApplication.translate(
                "ControlPanelWidget",
        ...
        translation_grid.setHorizontalSpacing(12)
        translation_grid.setVerticalSpacing(6)

        self.translation_target_lang_label = QLabel(
            QCoreApplication.translate("ControlPanelWidget", "目标语言：")
        )
        self.translation_target_lang_combo = QComboBox()
        # 逐项字面量添加（lupdate 提取）；userData 是写入翻译 prompt 的
        # 人类可读语言名（与 font_intel.translation 的 DEFAULT_TARGET_LANGUAGE 同风格）。
        for _tl_label, _tl_value in (
            ...
            self.translation_target_lang_combo.addItem(
                QCoreApplication.translate("ControlPanelWidget", _tl_label), _tl_value
            )
        ...
        self.translation_glossary_label.setToolTip(
            QCoreApplication.translate(
                "ControlPanelWidget",
        ...
        self.translation_glossary_edit = QLineEdit()
        self.translation_glossary_btn = QPushButton(
            QCoreApplication.translate("ControlPanelWidget", "浏览…")
        )
        self.translation_glossary_btn.setMinimumHeight(30)
        ...
        self.translation_context_label = QLabel(
            QCoreApplication.translate("ControlPanelWidget", "上下文行数：")
        )
        self.translation_context_edit = QLineEdit()
        self.translation_context_edit.setFixedWidth(60)
        self.translation_context_edit.setText("2")
        self.translation_max_chars_label = QLabel(
            QCoreApplication.translate("ControlPanelWidget", "最大行长：")
        )
        self.translation_max_chars_edit = QLineEdit()
        self.translation_max_chars_edit.setFixedWidth(60)
        self.translation_max_chars_edit.setText("42")
        numbers_row = QHBoxLayout()
        numbers_row.setSpacing(8)
        numbers_row.addWidget(self.translation_context_label)
        numbers_row.addWidget(self.translation_context_edit)
        numbers_row.addSpacing(12)
    ...
    @staticmethod
    def _parse_translation_int(value: object, default: int) -> int:
        """行内数字输入的安全解析：非法/负数回退默认，0 合法（= 关闭）。"""
        try:
    ...
    def _load_translation_from_settings(self) -> None:
        ...
        try:
            settings = QSettings()
            self.translation_group.setChecked(
                settings.value("translation/enabled", False, type=bool)
            )
            target = str(settings.value("translation/target_language", "") or "")
            if target:
            ...
            self.translation_context_edit.setText(
                str(
                    self._parse_translation_int(
                        settings.value("translation/context_lines", 2), 2
                    )
                )
            )
            self.translation_max_chars_edit.setText(
                str(
                    self._parse_translation_int(
                        settings.value("translation/max_line_chars", 42), 42
                    )
                )
            )
            self._update_translation_ui_state()
    ...
    def _save_translation_settings(self) -> None:
        ...
        settings.setValue(
            ...
            self._parse_translation_int(self.translation_context_edit.text(), 2),
        )
        settings.setValue(
    ...
    def _on_browse_glossary(self) -> None:
        path, _flt = QFileDialog.getOpenFileName(
            self,
            QCoreApplication.translate("ControlPanelWidget", "选择术语表 JSON 文件"),
            self.translation_glossary_edit.text().strip(),
            QCoreApplication.translate("ControlPanelWidget", "术语表 JSON (*.json)"),
        )
        if path:

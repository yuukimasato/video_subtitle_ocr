...
# 已知 reason → 展示文案。必须在 _reason_text() 里以**字符串字面量**直调
# QCoreApplication.translate（context=类名）：本版 pyside6-lupdate 的
# Python 解析器不识别 -tr-function-alias 别名，也不提取变量/常量实参，
# 包装或字典传参都会导致文案进不了 .ts（T2.5 实测）。与
# scan_review_dialog v2.6.0 的 i18n 修复同因同解。
def _reason_text(reason: str) -> str:
    key = reason or ""
    if key == "low_confidence":
        return QCoreApplication.translate("FontMapReviewDialog", "低置信度，建议人工确认")
    if key == "unrecognized":
        return QCoreApplication.translate("FontMapReviewDialog", "规则未能解析该行")
    if key == "hallucinated_name":
        return QCoreApplication.translate(
            "FontMapReviewDialog", "疑似幻觉字体名（归一化后不是原文行子串）")
    if key == "unknown_font_name":
        return QCoreApplication.translate("FontMapReviewDialog", "字体名未命中本地词表")
    if key == "llm_failed":
        return QCoreApplication.translate(
            "FontMapReviewDialog", "LLM 调用失败（有界退避耗尽），本行未结构化")
    if key == "llm_output_invalid":
        return QCoreApplication.translate(
            "FontMapReviewDialog", "LLM 输出不符合约定格式，本行未结构化")
    if key == "no_api_key":
        return QCoreApplication.translate("FontMapReviewDialog", "未配置 API Key，本行未结构化")
    if key == "empty":
        return QCoreApplication.translate("FontMapReviewDialog", "空行")
    if key == "comment":
        return QCoreApplication.translate("FontMapReviewDialog", "注释行")
    return reason or "-"
...
class _ItemRow(QWidget):
    ...
    def _summary_text(self) -> str:
        ...
        if suggestion and suggestion.get("kind") == "mapping":
            return QCoreApplication.translate("FontMapReviewDialog", "第 {0} 行：{1} → {2}").format(
        ...
        if suggestion:  # negative_mapping
            ...
            return QCoreApplication.translate(
                "FontMapReviewDialog", "第 {0} 行：{1} →（负映射）").format(line_no, names)
        line = str(self.item.get("line") or "")[:40]
        return QCoreApplication.translate("FontMapReviewDialog", "第 {0} 行：{1}").format(line_no, line)
    ...
    def _detail_text(self) -> str:
        ...
        if line:
            lines.append(QCoreApplication.translate("FontMapReviewDialog", "原文：{0}").format(line))
        suggestion = item.get("suggestion")
        if suggestion and suggestion.get("kind") == "mapping":
            lines.append(QCoreApplication.translate("FontMapReviewDialog", "建议：{0} → {1}").format(
            ...
            lines.append(QCoreApplication.translate(
                "FontMapReviewDialog", "建议：{0} 无日文本家对应").format(names))
        conf = item.get("confidence")
        conf_text = "—" if conf is None else f"{float(conf):.2f}"
        lines.append(QCoreApplication.translate(
            "FontMapReviewDialog", "类型：{0} ｜ 置信度：{1} ｜ 来源：{2} ｜ 文件：{3}").format(
            str(item.get("record_type") or "-"), conf_text,
            str(item.get("method") or "-") or "-",
            str(item.get("source_file") or "-") or "-"))
        lines.append(QCoreApplication.translate(
            "FontMapReviewDialog", "理由：{0}").format(_reason_text(str(item.get("reason") or ""))))
        if not self.actionable:
            lines.append(QCoreApplication.translate(
                "FontMapReviewDialog", "（该条目不写库；如需修正请在源表修订后重跑导入）"))
        return "\n".join(lines)
...
class FontMapReviewDialog(QDialog):
    ...
    def __init__(self, items, parent=None):
        super().__init__(parent)
        self.setWindowTitle(
            QCoreApplication.translate("FontMapReviewDialog", "字体映射复核"))
        self.resize(760, 600)
        self.setModal(True)
        ...
        if self._items:
            ...
            content_layout.addWidget(QLabel(
                QCoreApplication.translate("FontMapReviewDialog", "无待复核记录。"), content))
        ...
        buttons.button(QDialogButtonBox.Ok).setText(
            QCoreApplication.translate("FontMapReviewDialog", "应用"))
        buttons.button(QDialogButtonBox.Cancel).setText(
            QCoreApplication.translate("FontMapReviewDialog", "取消"))

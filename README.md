...
- **移动文字轨迹字幕**：手机屏幕、运动招牌等**画面内移动文字**的专属管线——
  ...
  - **时间保真门控**：轨迹接管静态行前逐区间比对内容保真度（比较窗口归一到静态证据跨度、端点对称去噪），把已滚出屏幕的标题行「冻结」到整段的错误接管会被拒绝并保留静态输出。
  - **字形证据运动判定**：参考帧字形掩码优先判定「文字动没动」——横向移动背景上的静止文字漂移 ≤1px 不再被背景拖出假 `\move`；字形证据不足时回退整块模板匹配并留痕。
  - **亮度自适应**：逐帧测量文字平面明暗，为轨迹事件追加 `\1c`/`\alpha` `\t` 标签链——字幕颜色与透明度忠实跟随屏幕变暗/变亮（如手机息屏）；遮罩原文字策略的有色遮罩与文字共用同一亮度测量，暗屏下不再比背景亮。
  - **遮挡蒙版与手部轮廓**：检测手部等遮挡物覆盖文字平面的帧，追加 `\iclip` 逆向蒙版，字幕不再渲染到遮挡物上；遮挡采样证据全量保留、按命中帧离散切片（不再二次抽样漏采），静止画面下细化限定在平面检测命中窗口内，前景轮廓细化保留指缝洞。
...
- **场景文字显示策略**（每 ROI 独立选择）：画面文字可按需选择 **叠加**（默认重渲染原位）/ **遮罩原文字**（`\p1` 纯色遮罩盖字 + layer 1 重渲染）/ **仅遮罩**（识别文本写成 Comment 行，供排版覆写译文）/ **外置展示框** / **空白区放置**；可用性不足时按预设回退链自动降级并留痕。策略按文字实际活动时间片布局输出（同屏多行聊天不再被压成单条或截为零时长）；无样式模板时按识别行框约束逐行 `\fs`，字号不超出原文字实际大小。

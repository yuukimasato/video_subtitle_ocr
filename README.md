...
- **多语言识别**：内置 12 种识别语言（简体中文、繁體中文、English、日本語、한국어、Русский、Français、Deutsch、Italiano、Español、Português、العربية）。简/繁中、英、日及拉丁语系使用 PP-OCRv6 模型（无需按语言额外下载），韩/俄/阿拉伯语自动回落 PP-OCRv5 多语言模型；全部语言经真实引擎逐字验证。
- **识别质量增强（可选）**：
...
| `--roi-file` | 从 JSON 文件读取 ROI（含 GUI 导出的逐 ROI 开关：位置标签/亮度自适应/遮挡蒙版/场景策略） |

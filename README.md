...
```
    ...
    pip install -U pip
    pip install -r opt/apps/video_subtitle_ocr/requirements.txt
```
    **注意**:
    - **CPU/GPU 选择**：
      - 默认安装 `requirements.txt` 会使用 **CPU 版** `paddlepaddle`。
    ...
    - **OpenCV 版本选择（避免冲突）**：
      ...
      - 本项目默认依赖 `opencv-python`；如你确实需要 contrib 模块，请改装 `opencv-contrib-python` 并确保只保留一个 OpenCV 包。
    - **OpenCV GPU**：
...
```
    python3 opt/apps/video_subtitle_ocr/main.py
```

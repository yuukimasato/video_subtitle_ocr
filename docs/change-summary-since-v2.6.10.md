...
| ④ | `271c91c`…`24f3413` | 2.7.1 | 4K 实测驱动的修复/内存/锚定/可观测性轮次（原「工作区未提交」部分入库） |
| 终点 | 本次提交 | 2.7.2 | 步进/静止体制分段（`core/step_segmentation.py`，hold 段逐段 `\pos`） |
...
>
> **2026-09-21 更新**：上表「终点」原指 2026-09-20 的工作区改动，现已随
> v2.7.1 发布入库（至 `24f3413`）；其上 2.7.2 新增步进/静止体制分段与
> 复核补采，逐条叙述见 `CHANGELOG.md` 2.7.2 一节。
...
> **2026-09-20 修复轮更新**：下列前两条已在当日修复——`verify_screen_pose`
> 的歌词条位姿破坏由 ROI 包含门控解决（11.mp4/DMG 产物逐字节不变，4K
> 歌词条恢复正确位姿）；4K OCR 推理内存由检测阶段工作分辨率封顶缓解
> （单张 4K 峰值 3461 → 2038MB，识别裁剪仍取原图）。详见 `CHANGELOG.md`
> 「未发布」节与 `docs/testing.md` 2026-09-20 第二轮。
>
> **同日第三轮（残留收尾）**：门控「回退全局峰」分支不可观测的问题由
> `LineVerifyReport.n_roi_fallback` 计数解决（滚出仍可见与消失段假锁两
> 种形态共用，零采纳行为变化）；「消失段校正不得移出 ROI」的行为约束经
> 评估**缓行**（稀疏采样下按帧门控会误伤 DMG 滚出形态），待真实素材暴露
> 后以该计数定位单独立项。
